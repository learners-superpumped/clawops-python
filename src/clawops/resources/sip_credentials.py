...
from ..types.sip import SipCredential


class _SipCredentialListResponse(BaseModel):
    data: list[SipCredential]


class SipCredentials(SyncAPIResource):
    """SIP 단말(credential) 리소스 — 조회 전용.

    생성/수정/삭제는 대시보드 또는 REST API 를 사용. 본 SDK 는 softphone 라우팅
    설정에 필요한 credential id 를 확인하기 위한 list/get 만 제공한다.
    """
    ...
    def list(self, *, status: Literal["active", "disabled"] | None = None,
             ...
             timeout: float | None = None) -> list[SipCredential]:
        """등록된 SIP 단말 목록을 조회합니다. (password/ha1 미포함)
        ...
        result = self._client._get(
            f"{self._base_path}/sip-credentials", cast_to=_SipCredentialListResponse,
    ...
    def get(self, credential_id: str, *, extra_headers: dict[str, str] | None = None,
            timeout: float | None = None) -> SipCredential:
        """SIP 단말 단건을 id 로 조회합니다. (password/ha1 미포함)"""
        return self._client._get(
            f"{self._base_path}/sip-credentials/{credential_id}", cast_to=SipCredential,
            extra_headers=extra_headers, timeout=timeout,
        )


class AsyncSipCredentials(AsyncAPIResource):
    """SIP 단말(credential) 리소스 (비동기) — 조회 전용."""

    async def list(self, *, status: Literal["active", "disabled"] | None = None,
                   page: int | None = None, page_size: int | None = None,
                   extra_headers: dict[str, str] | None = None, extra_query: dict[str, object] | None = None,
                   timeout: float | None = None) -> list[SipCredential]:
        """등록된 SIP 단말 목록을 비동기로 조회합니다."""
        query = strip_not_given({"status": status, "page": page, "pageSize": page_size})
        merged_query = {**query, **(extra_query or {})}
