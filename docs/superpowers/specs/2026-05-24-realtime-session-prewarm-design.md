...
## 한계 (current implementation, clawops-python)

- **ClawOpsAgent 1 인스턴스 = 동시 outbound 1건 가정** — `self._session` 단일 인스턴스를
  prewarm 시 단일 `_BufferingCall` 로 점유한다. 같은 인스턴스로 동시 outbound 통화 발신 시
  prewarm race (두 번째 호출이 첫 번째의 BufferingCall 을 덮어쓰거나, attach 가 잘못된 통화로
  부착될 가능성) 가 존재한다. 다중 동시 outbound 가 필요하면 통화별 인스턴스 분리 또는
  session factory 도입 (후속).
- **Session 타입별 효과** — Realtime (OpenAI / Gemini) 에서 LLM WS handshake +
  session.update + greeting prebuffer 가 모두 prewarm 단계에서 선행되므로 latency 절감
  효과가 가장 크다. `PipelineSession` 은 STT / LLM / TTS 자원이 lazy 연결되어, prewarm 은
  STT 루프 기동과 greeting kickoff 정도에 그쳐 latency 절감 효과가 제한적이다.
